...
// Helper functions for Cheqing Account class

// Displays Cheqing Account objects in a  vector of Cheqing Account objects 
void display(const std::vector<Cheqing_Account> &accounts) {
    std::cout << "\n=== Cheqing Accounts ====================================" << std::endl;
    for (const auto &acc: accounts) 
        std::cout << acc << std::endl;
}

// Deposits supplied amount to each Cheqing Account object in the vector
void deposit(std::vector<Cheqing_Account> &accounts, double amount) {
    std::cout << "\n=== Depositing to Cheqing Accounts ==========================" << std::endl;
    for (auto &acc:accounts)  {
        if (acc.deposit(amount)) 
            std::cout << "Deposited " << amount << " to " << acc << std::endl;
        else
...
// Withdraw supplied amount from each Cheqing Account object in the vector
void withdraw(std::vector<Cheqing_Account> &accounts, double amount) {
    std::cout << "\n=== Withdrawing from Cheqing Accounts ======================" << std::endl;
    for (auto &acc:accounts)  {
        if (acc.withdraw(amount)) 
            std::cout << "Withdrew " << amount << " from " << acc << std::endl;
        else
